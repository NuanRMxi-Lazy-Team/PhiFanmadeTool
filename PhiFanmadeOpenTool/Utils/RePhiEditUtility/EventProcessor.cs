...
internal static class EventProcessor
{
    ...
    public static List<Event<T>> EventMerge<T>(
        List<Event<T>> toEvents, List<Event<T>> formEvents)
    {
        ...
        foreach (var formEvent in formEventsCopy)
        {
            foreach (var toEvent in toEventsCopy)
            {
                ...
                {
                    /*
                    RePhiEditUtility.OnWarning.Invoke(
        ...
        if (overlapFound)
        {
            var newEvents = new List<Event<T>>();
            ...
            var cutLength = new Beat(0.015625d);
