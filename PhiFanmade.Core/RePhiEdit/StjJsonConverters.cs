...
namespace PhiFanmade.Core.RePhiEdit
{
    ...
    {
        public override List<Event<byte[]>> Read(ref Utf8JsonReader reader, Type typeToConvert,
            JsonSerializerOptions options)
        {
            ...
            using (var doc = JsonDocument.ParseValue(ref reader))
            {
                foreach (var item in doc.RootElement.EnumerateArray())
                {
                    ...
                    {
                        var ints = JsonSerializer.Deserialize<int[]>(startEl.GetRawText(), options) ??
                                   Array.Empty<int>();
                    ...
                    if (item.TryGetProperty("bezier", out var bezierEl))
                        evt.IsBezier = bezierEl.ValueKind == JsonValueKind.Number
                            ? bezierEl.GetInt32() == 1
                            : bezierEl.GetBoolean();
                    if (item.TryGetProperty("bezierPoints", out var bezierPointsEl))
                        evt.BezierPoints = JsonSerializer.Deserialize<float[]>(bezierPointsEl.GetRawText(), options) ??
                                           new float[4];
