...
internal static class FatherUnbindAsyncProcessor
{
        ...
        List<Rpe.JudgeLine> allJudgeLines, double precision = 64d, double tolerance = 5d)
    {
        ...
        try
        {
            ...
            var targetXBag = new ConcurrentBag<List<Rpe.Event<float>>>();
            var targetYBag = new ConcurrentBag<List<Rpe.Event<float>>>();
            ...
            // 并行处理父线事件
            var fatherXBag = new ConcurrentBag<List<Rpe.Event<float>>>();
            var fatherYBag = new ConcurrentBag<List<Rpe.Event<float>>>();
            var fatherRotateBag = new ConcurrentBag<List<Rpe.Event<float>>>();
