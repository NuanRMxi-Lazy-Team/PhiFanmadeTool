...
public static class RePhiEditHelper
{
    ...
    {
        public static float ToPeX(float RpeX)
        {
            ...
            return (RpeX - rpeMin) / (rpeMax - rpeMin) * (peMax - peMin) + peMin;
        }

        public static float ToPeY(float RpeY)
        {
            ...
            return (RpeY - rpeMin) / (rpeMax - rpeMin) * (peMax - peMin) + peMin;
        }
    }
    ...
    public static Rpe.JudgeLine FatherUnbind(int targetJudgeLineIndex, List<Rpe.JudgeLine> allJudgeLines,
    ...
    public static List<Rpe.Event<T>> EventMerge<T>(
        List<Rpe.Event<T>> toEvents, List<Rpe.Event<T>> formEvents, double precision = 64d,
        double tolerance = 5d)
