...
internal static class EventProcessor
{
        ...
        double tolerance = 5d)
    {
        ...
        if (overlapFound)
        {
            ...
            var cutLength = new Beat(1d / precision);
            var result = new List<Rpe.Event<T>>();
            ...
            foreach (var (start, end) in overlapIntervals)
            {
                ...
                var currentBeat = start;
                // 初始化跨事件检测所需的当前拍追踪变量
                var toEventAtCurrent = toEventsCopy
                ...
                while (currentBeat < end)
                {
                    ...
                    // 在 nextBeat 处，获取 to/form 的当前值
                    var toEventAtNext = toEventsCopy
                        .Where(e => e.StartBeat <= nextBeat && e.EndBeat >= nextBeat)
                        .MaxBy(e => e.StartBeat);
                    var formEventAtNext = formEventsCopy
                        .Where(e => e.StartBeat <= nextBeat && e.EndBeat >= nextBeat)
                        .MaxBy(e => e.StartBeat);

                    // 检测跨事件采样：若任意一侧的事件发生变化，则在 currentBeat 处强制切割，避免虚假连贯
                    bool crossEvent = !ReferenceEquals(toEventAtNext, toEventAtCurrent) ||
                                      !ReferenceEquals(formEventAtNext, formEventAtCurrent);
                    if (crossEvent && currentBeat > segmentStart)
                    {
                        result.Add(new Rpe.Event<T>
                        {
                    ...
                    // toValueAtNext / formValueAtNext  ── 用于当前段的 EndValue
                    //   若当前事件延伸至 nextBeat，取其数值（outgoing）；
                    //   否则取 nextBeat 前最近一个已结束事件的 EndValue（gap / 边界左侧）。
                    //   绝不使用从 nextBeat 开始的新事件的 StartValue，以避免虚假连贯。
                    ...
                    // 用 segmentStart 到 end 的线性预测
                    var toAtEnd = toEventsCopy
                        .Where(e => e.StartBeat <= end && e.EndBeat >= end)
                        .MaxBy(e => e.StartBeat);
                    var formAtEnd = formEventsCopy
                        .Where(e => e.StartBeat <= end && e.EndBeat >= end)
                        .MaxBy(e => e.StartBeat);
    ...
    /// </summary>
    internal static List<Rpe.Event<T>>? RemoveUnlessEvent<T>(List<Rpe.Event<T>>? events)
    {
